...
-- Description: This query gets the first weight and height and BMI
...
CREATE TABLE DATABASE.height_weight_bmi
AS
...
(
    ...
    MAX(case when parameter = 'HEIGHT' then first_valuenum else NULL end) AS height,
    MAX(case when parameter = 'WEIGHT' then first_valuenum else NULL end) AS weight
    FROM SingleParameters
    GROUP BY subject_id, icustay_id
)
--select * from PivotParameters
SELECT f.icustay_id, f.subject_id,
ROUND(cast(f.height as double), 2) AS height,
ROUND(cast(f.weight as double), 2) AS weight,
CASE
    WHEN f.height IS NOT NULL AND f.weight IS NOT NULL
    THEN f.weight / (f.height / 100 * f.height / 100)
    ELSE NULL
END AS bmi
